            ...
            for requirement in body_sent_from_optimizer:
                ...
                if sal_body:
                    ...
                    for node in nodes_by_requirement:
                        try:
                            if ("cloud" in node and node["cloud"]["id"] != "edge" and "api" in node["cloud"] and "providerName" in node["cloud"]["api"] and "aws-ec2" == node["cloud"]["api"]["providerName"] and node["hardware"]["name"] not in VALID_AWS_INSTANCE_TYPES):
